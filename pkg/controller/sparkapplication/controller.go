...
func (c *Controller) syncSparkApplication(key string) error {
	...
	case v1beta2.NewState:
		...
		} else if shouldSuspend(appCopy) {
			glog.V(5).Infof("SparkApplication %s/%s was suspended", appCopy.Namespace, appCopy.Name)
		} else {
...
// Helper func to determine whether to suspend the SparkApplication
